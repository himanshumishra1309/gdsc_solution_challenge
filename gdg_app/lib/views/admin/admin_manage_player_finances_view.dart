...
class _AdminFinancialViewState extends State<AdminFinancialView> {
  ...
  String _userEmail = "";
  String? _userAvatar;
  bool _isLoading = true; // Changed from false to true

  // Replace hard-coded data with these variables
  List<Map<String, dynamic>> _players = [];
  Map<String, Map<String, dynamic>> _financeCategories = {};
  List<Map<String, dynamic>> _sportFilters = [];
  Map<String, String> _budgetSummary = {};
  List<Map<String, dynamic>> _transferPurposes = [];
  ...
  // Add this method to the _AdminFinancialViewState class
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      // Load finance data from JSON
      final String jsonString = await rootBundle
          .loadString('assets/json_files/player_finance_data.json');
      ...
      setState(() {
        ...
        // Process finance categories with type conversion
        _financeCategories = {};
        (data['financeCategories'] as Map).forEach((key, value) {
          final Map<String, dynamic> categoryData =
              Map<String, dynamic>.from(value);

          // Pre-convert color string to Color object
          if (categoryData.containsKey('color') &&
              categoryData['color'] is String) {
            final colorString = categoryData['color'] as String;
            categoryData['color'] = getColorFromString(colorString);
          }

          // Pre-convert icon string to IconData object
          if (categoryData.containsKey('icon') &&
              categoryData['icon'] is String) {
            final iconString = categoryData['icon'] as String;
            categoryData['icon'] = getIconFromString(iconString);
          }

          _financeCategories[key.toString()] = categoryData;
        });

        _sportFilters = List<Map<String, dynamic>>.from(data['sportFilters']);
        _budgetSummary = Map<String, String>.from(data['budgetSummary']);
        _transferPurposes =
            List<Map<String, dynamic>>.from(data['transferPurposes']);
      });
    } catch (e) {
                          ...
                          : Scrollbar(
                              ...
                              child: // Categories list in the _showPlayerFinance method
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  // Convert string to Color and IconData objects
                                  final Color iconColor = getColorFromString(
                                      data['color'].toString());
                                  final IconData iconData = getIconFromString(
                                      data['icon'].toString());
                                                ...
                                                onPressed: () {
                                                  ...
                                                  if (newValue != null) {
                                                    ...
                                                    ScaffoldMessenger.of(
                                                      ...
                                                      SnackBar(
                                                        content: Text(
                                                            'Updated $category to ₹${newValue.toStringAsFixed(2)}'),
                                                        behavior:
                                                            SnackBarBehavior
                                                                .floating,
                                                        backgroundColor:
                          ...
                          ElevatedButton.icon(
                            onPressed: () {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text(
                                      'Financial changes saved successfully'),
  ...
  Widget _buildFinanceSummaryCard(
      String label, double amount, IconData icon, Color textColor) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          // Replace this in your build method
          Container(
              ...
              children: [
                ...
                Row(
                  children: [
                    _buildStatCard(
                        'Total Budget',
                        _budgetSummary['totalBudget'] ?? '',
                        Icons.account_balance,
                        Colors.amber),
                    const SizedBox(width: 12),
                    _buildStatCard(
                        'Allocated',
                        _budgetSummary['allocated'] ?? '',
                        Icons.people,
                        Colors.green),
                    const SizedBox(width: 12),
                    _buildStatCard(
                        'Available',
                        _budgetSummary['available'] ?? '',
                        Icons.savings,
                        Colors.blue),
                  ],
                ),
            ...
            child: filteredPlayers.isEmpty
                ...
                : GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final progress =
                          player['remaining'] / player['total_funds'];
  ...
  // Keep only the JSON data version and remove the hardcoded version
  Widget _buildSportFilterChip(Map<String, dynamic> sport) {
    final isSelected = _selectedSport == sport['name'];
    final icon = getIconFromString(sport['icon']);
      ...
      child: InkWell(
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              padding:
                  const EdgeInsets.fromLTRB(10, 8, 10, 8), // Reduced padding
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                mainAxisSize:
                ...
                builder: (context, setState) {
                  return Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      // In the _showTransferDialog method
                      Wrap(
                        spacing: 8,
                        runSpacing: 8,
                        children: _transferPurposes.map((purpose) {
                          final icon = getIconFromString(purpose['icon']);
                          return _buildPurposeChip(
                              purpose['name'], icon, selectedPurpose, (value) {
            ...
            onPressed: () {
              ...
              if (amount != null && amount > 0) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                        'Successfully transferred ${formatter.format(amount)} to ${player['name']}'),
                    backgroundColor: Colors.green,
                    behavior: SnackBarBehavior.floating,
                  ),
                );
  ...
  Widget _buildPurposeChip(String label, IconData icon, String selectedValue,
