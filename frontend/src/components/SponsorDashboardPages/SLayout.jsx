...
function SLayout({ userType, navItems, children }) {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      <div className="flex flex-col w-72 bg-yellow-400 text-white border-r">
        <div className="flex flex-col items-center p-6 bg-yellow-500 border-b-2 border-darkblue-700">
          ...
          <h2 className="text-2xl font-semibold text-white">Nike</h2>
        ...
        <nav className="flex-1 p-4">
          {navItems.map((item, index) => {
            ...
            return (
              <Link key={index} to={`/sponsor-dashboard/${item.path}`}>
                <Button
                  variant="ghost"
