...
export default function SponsorSignUp() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-green-200 via-teal-200 to-blue-200">
      ...
      <div className={`relative w-[880px] max-w-full min-h-[700px] bg-white rounded-3xl shadow-2xl overflow-hidden transition-all duration-1000 ease-in-out ${isActive ? "active" : ""}`}>
        
        <div className={`absolute top-0 left-0 w-1/2 h-full flex flex-col items-center justify-center px-10 transition-all duration-1000 ease-in-out ${isActive ? "translate-x-full opacity-100 z-10" : "opacity-0 z-[-1]"}`}>
          ...
          <button onClick={handleSignUp} className="mt-6 px-6 py-2 bg-green-700 text-white rounded-md hover:bg-green-800 transition">Sign Up</button>
        </div>

        <div className={`absolute top-0 left-0 w-1/2 h-full flex flex-col items-center justify-center px-10 transition-all duration-1000 ease-in-out ${isActive ? "translate-x-full opacity-0 z-[-1]" : "opacity-100 z-10"}`}>
          ...
          <button onClick={handleSignIn} className="mt-5 px-6 py-2 bg-green-700 text-white rounded-md hover:bg-green-800 transition">Sign In</button>
        </div>

        <div className={`absolute top-0 left-1/2 w-1/2 h-full flex items-center justify-center bg-green-700 text-white transition-all duration-1000 ease-in-out ${isActive ? "-translate-x-full rounded-tr-3xl rounded-br-3xl" : "rounded-tl-3xl rounded-bl-3xl"}`}>
          <div className="text-center">
            <h1 className="text-3xl font-semibold">{isActive ? "Welcome Back!" : "Join as a Sponsor!"}</h1>
            <p className="mt-5">{isActive ? "Enter your details to sign in." : "Register your details to sign up."}</p>
            <button onClick={() => setIsActive(!isActive)} className="mt-9 px-6 py-2 bg-transparent border border-white rounded-md hover:bg-white hover:text-gray-800 transition">{isActive ? "Sign In" : "Sign Up"}</button>
          </div>
        </div>
      </div>
    </div>
